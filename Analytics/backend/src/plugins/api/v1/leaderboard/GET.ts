	...
	fastify.get<{ Querystring: Query }>("/", async (req, res) => {
		...
		try {
			...
			const { rows } = await fastify.pg.query(
				"SELECT DISTINCT ON (leaderboard.player_id) leaderboard.id, game.portfolio_value, player.name FROM leaderboard INNER JOIN game ON leaderboard.game_id = game.id INNER JOIN player ON leaderboard.player_id = player.id ORDER BY leaderboard.player_id DESC, portfolio_value DESC LIMIT 10 OFFSET $1",
				[(page - 1) * 10]
			);
