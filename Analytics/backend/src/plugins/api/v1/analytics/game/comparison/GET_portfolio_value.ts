	...
	fastify.get("/portfolio_value", async (req, res) => {
		...
		try {
			const { rows: allPlayersRows } = await fastify.pg.query<{ portfolio_value: number }>(
				"SELECT ROUND(AVG(portfolio_value))::INT AS portfolio_value FROM game WHERE DATE(timestamp) != CURRENT_DATE AND turns > 10;"
			);
			...
			const { rows: topPlayersRows } = await fastify.pg.query<{ portfolio_value: number }>(
				`SELECT
					...
					FROM (SELECT
							portfolio_value
							FROM game
							WHERE DATE(timestamp) != CURRENT_DATE AND turns > 10
							ORDER BY portfolio_value DESC
							LIMIT (
								(SELECT count(*) FROM game WHERE DATE(timestamp) != CURRENT_DATE AND turns > 10) / 10)
								)t;`
			);
