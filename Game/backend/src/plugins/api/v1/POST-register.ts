...
export default async function (fastify: FastifyInstance) {
	fastify.post<{ Body: Body }>("/register", async (request, reply) => {
		try {
			const { deviceID, displayName, email, firstName, lastName, mobile, agreeTerms } =
				request.body;
			...
			const { rowCount: playerExists, rows } = await fastify.pg.query<{
				...
				`SELECT id, name, first_name, last_name, email, mobile FROM player WHERE
				(LOWER(first_name) = $1 AND LOWER(last_name) = $2)
				OR LOWER(email) = $3
				OR mobile = $4;`,
				[firstName.toLowerCase(), lastName.toLowerCase(), email.toLowerCase(), mobile]
			);
			...
			if (playerExists === 0) {
				...
				await fastify.pg.query(
					...
					[displayName, firstName, lastName, email, mobile, currentPlayerID]
				);
