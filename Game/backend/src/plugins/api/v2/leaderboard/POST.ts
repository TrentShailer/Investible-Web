...
export default async function (fastify: FastifyInstance) {
	fastify.post<{ Body: Body }>("/", async (request, reply) => {
		try {
			...
			const { rowCount: playerExists } = await fastify.pg.query<{ id: string }>(
			...
			if (playerExists === 0) {
				...
				if (!currentPlayer[0].player_id) {
					...
					const { rowCount: playerExists, rows: playerRows } = await fastify.pg.query<{
						id: string;
					}>(
						`SELECT id FROM player WHERE email = $1 OR (first_name = $2 AND last_name = $3) OR mobile = $4;`,
					...
					// if a player does not exist, create a new player
						...
						await fastify.pg.query(
							`INSERT INTO player (id, first_name, last_name, email, mobile, display_name) VALUES ($1, $2, $3, $4, $5, $6);`,
							[playerID, first_name, last_name, email, mobile, displayName]
						);
						await fastify.pg.query(`UPDATE device SET player_id = $1 WHERE id = $2;`, [
							playerID,
							deviceID,
